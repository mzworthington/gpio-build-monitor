...
def create_app(
    ...
    app = web.Application()
    register_webhook_routes(app, signal, secrets, providers)
    return app
...
def register_webhook_routes(
    ...
    for provider in providers if providers is not None else get_all():
        handler = partial(_webhook, provider=provider)
        app.router.add_post(provider.path, handler)
        app.router.add_post(f"/api{provider.path}", handler)

...
from monitor.webhooks.constants import WebhookSecrets
from monitor.webhooks.server import register_webhook_routes

WEB_DIR = Path(__file__).resolve().parents[3] / "device" / "web" / "public"
...
class WebSocketStatusOutput:
    ...
    def __init__(
        ...
        webhook_secrets: WebhookSecrets | None = None,
    ):
        self._host = host
        self._port = port
        ...
        self._webhook_signal = webhook_signal
        self._webhook_secrets = webhook_secrets
    ...
    async def start(self) -> None:
        ...
        app.router.add_get("/api/ws", self._websocket_handler)
        app.router.add_get("/api/status", self._status_handler)
        app.router.add_get("/posthog-config.json", self._posthog_config_handler)
        app.router.add_get("/api/push/vapid-public-key", self._vapid_public_key_handler)
        if self._webhook_signal is not None and self._webhook_secrets is not None:
            register_webhook_routes(app, self._webhook_signal, self._webhook_secrets)
        if self._web_dir.is_dir():
            app.router.add_get("/", self._index_handler)
            for name, content_type in (
                ("monitor.js", "application/javascript"),
                ("countdown.js", "application/javascript"),
                ("posthog.js", "application/javascript"),
                ("styles.css", "text/css"),
    ...
    async def _vapid_public_key_handler(self, _request: web.Request) -> web.Response:
        return web.json_response({"publicKey": None})

    async def _posthog_config_handler(self, _request: web.Request) -> web.Response:
        api_key = os.environ.get("POSTHOG_TOKEN", "").strip()
        headers = {"Cache-Control": "no-store"}
        if not api_key:
            return web.json_response({"enabled": False}, headers=headers)
        api_host = os.environ.get("POSTHOG_HOST", "").strip() or "https://a.mzworthington.co.uk"
        return web.json_response(
            {"enabled": True, "apiKey": api_key, "apiHost": api_host},
            headers=headers,
        )

...
def test_last_valid_payload_keeps_builds_when_next_is_empty():
    previous = {
        "status": "FAIL",
        "is_running": False,
        "fetching": False,
    ...
    next_payload = {"status": "NONE", "is_running": False, "fetching": False, "builds": []}
    kept = last_valid_payload(previous, next_payload)
    text = plugin_output(kept, dashboard_url="https://monitor.example")
    assert "No builds yet" not in text
    assert "CI" in text

...
```shell
cp monitor/device/eink/apps/monitor/platformio.local.ini.example \
   monitor/device/eink/crosspoint/platformio.local.ini
```
...
```shell
git -C monitor/device/eink/crosspoint checkout -- .
git -C monitor/device/eink/crosspoint submodule update --init --recursive
```

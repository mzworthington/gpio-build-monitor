...
export class StatusHub implements DurableObject {
  ...
  constructor(state: DurableObjectState, env: Env) {
    ...
    void this.state.blockConcurrencyWhile(async () => {
      const stored = await this.state.storage.get<StatusPayload>(LAST_PAYLOAD_KEY);
      this.payload = keepLastValidPayload(stored, this.payload);
    });
  }
  ...
  async refresh(): Promise<void> {
    ...
    this.payload = keepLastValidPayload(this.payload, {
      type: 'status',
      fetching: false,
      status,
      is_running,
    ...
    await this.state.storage.put(
      LAST_STATUS_KEY,
      statusToRemember(previous, this.payload.status, this.payload.is_running),
    );
    if (this.payload.builds.length > 0) {
      await this.state.storage.put(LAST_PAYLOAD_KEY, this.payload);
    }
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (isPythonApiPath(url.pathname) && env.PYTHON_API_ORIGIN) {
